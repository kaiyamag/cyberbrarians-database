...
def add_course():
    ...
    course = Course(request.json['title'])      # NOTE: Do we need to add all fields?
    result = coursedb.insert_course(course)
    
    return jsonify({"status": "success", "id": result['course_id']}), 200
...
def delete_course(course_id):
    ...
    return jsonify({"status": "success", "id": course_id}), 200
